...
public class ChildMasterController {
	...
	public long getCountChild() {
		...
		long a = child_repository.findCount();
		System.out.println(a);
		return a;
	}

	@GetMapping("/child/top100")
	public List<child_master> getAllChildDetails() {
		System.out.println("Get top 100 child details...");

		List<child_master> a = child_repository.findTopChild();
		System.out.println(a);
		return a;
	}

	@PostMapping("/child/top100/post")
	public List<child_master> getAllChildDetailsPost() {
		System.out.println("Get top 100 child details...");
	...
	@GetMapping("workplan/facility/{selectState}/{selectDistrict}/{selectBlock}/{start_date}/{end_date}")
	public List<child_master> WorkplanlistPhc(@PathVariable String selectState, @PathVariable String selectDistrict,
			@PathVariable String selectBlock, @PathVariable String start_date, @PathVariable String end_date) throws ParseException {
		System.out.println("Get Phc List - " + selectBlock);
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
		...
		LocalDate start_date_local = LocalDate.fromMillisSinceEpoch(d1.getTime());
		LocalDate end_date_local = LocalDate.fromMillisSinceEpoch(d2.getTime());
		List<child_master> c = child_repository.findPhcWorkplan(selectState, selectDistrict, selectBlock,start_date_local,end_date_local);
		System.out.println(c);
		return c;
	}
